...
На маршрутизаторе R2 выполним команду no passive-interface, чтобы маршрутизатор отправлял и получал обновления маршрутизации OSPF. После ввода этой команды появится уведомление о том, что были установлены соседские отношения смежности с маршрутизатором R1. 

Повторно выполним команды show ip route и show ip ospf neighbor на маршрутизаторах R1 и R3 и найдите маршрут к сети 192.168.2.0/24. 

![](https://github.com/rayakhin/OTUS_Neteng/blob/master/Homework/HW_5/R1-3_NEIGHBOR.PNG)


Из данного вывода можно судить, что R3 для маршрута к сети 192.168.2.0/24 использует интерфейс S1/2. Cуммарная метрика стоимости для сети 192.168.2.0/24 на R3 равна 74. И видно что соседство с R1 и R3, есть.


<h4>5.Изменение метрик OSPF</h4>
 Изменим метрики OSPF с помощью команд auto-cost reference-bandwidth, bandwidth и ip ospf cost.
...
![](https://github.com/rayakhin/OTUS_Neteng/blob/master/Homework/HW_5/AUTO-COST.PNG)

аналогично повторим для R2 и R3.

Выполним команду show ip ospf interface, чтобы просмотреть новую стоимость интерфейса е0/0 на R3 и интерфейса S1/3 на R1.

![](https://github.com/rayakhin/OTUS_Neteng/blob/master/Homework/HW_5/AUTO-COST2.PNG)

Повторно введем команду show ip route ospf для просмотра новой накопленной стоимости для маршрута 192.168.3.0/24 (1000 + 6476 = 7486).

![](https://github.com/rayakhin/OTUS_Neteng/blob/master/Homework/HW_5/ROUTE_OSPF.PNG)
...
В целом изменения на эталонной пропускной способность OSPF по умолчанию, потребуется когда будут интерфейсы превышающие емкость
100Мбит.

Попробуем изменить пропускную способность для интерфейса. 
...
Выполним команду show interface s1/2на маршрутизаторе R1, чтобы просмотреть текущую пропускную способность на интерфейсе S0/0/0. Хотя тактовая частота (скорость передачи данных) для этого интерфейса была задана равной 128 Кбит/с, пропускная способность по-прежнему показывается как 1544 Кбит/с. 

![](https://github.com/rayakhin/OTUS_Neteng/blob/master/Homework/HW_5/INT_R1.PNG)

Введем команду show ip route ospf на маршрутизаторе R1 для просмотра накопленной стоимости маршрута к сети 192.168.23.0/24 с использованием интерфейса S0/0/0. Обратим внимание, что к сети 192.168.23.0/24 есть два маршрута с равной стоимостью (128): один через интерфейс S0/0/0, а другой через S0/0/1. 


![](https://github.com/rayakhin/OTUS_Neteng/blob/master/Homework/HW_5/IP_ROUTE_OSPF1.PNG)

Выполним команду bandwidth 128, чтобы установить для интерфейса S1/2 пропускную способность равной 128 Кбит/c.
И повторно введите команду show ip route ospf.

![](https://github.com/rayakhin/OTUS_Neteng/blob/master/Homework/HW_5/IP_ROUTE_OSPF2.PNG)

Изменим на маршрутизаторе R1 пропускную способность для интерфейса S1/3 на значение, равное значению для интерфейса S1/2.
И введем команду show ip route ospf для просмотра накопленной стоимости обоих маршрутов к сети 192.168.23.0/24

![](https://github.com/rayakhin/OTUS_Neteng/blob/master/Homework/HW_5/BAND.PNG)

Cтоимость маршрутов от маршрутизатора R1 для 192.168.3.0/24 равна 65535(стоимость канала за S1/3 на R1)+1000( стоимость канала за e0/0 на R3), а для 192.168.23.0/30 рассчитана как сумма  двух каналов(либо от R1 и R2 если рассматривать со стороны S1/2) то есть 
65535+6476.

Выполним команду bandwidth 128 для всех остальных последовательных интерфейсов в топологии. 
Теперь новая суммарная стоимость для сети 192.168.23.0/24 на R1 (так как стоимость всех последовательных
портов стало 66535) стала  131070. 
...
Выполним команду ip ospf cost 1565 для интерфейса S1/3 маршрутизатора R1, и повторно введем команду show ip route ospf.

![](https://github.com/rayakhin/OTUS_Neteng/blob/master/Homework/HW_5/OSPF_COST.PNG)

 Маршрут к сети 192.168.3.0/24 от маршрутизатора R1 теперь проходит через R3, так по стоимости он стал более предпочительным. 
